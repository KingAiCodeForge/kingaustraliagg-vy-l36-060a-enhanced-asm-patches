...
;   - TIC3 (24X Crank) vector → $00C4 in RAM
...
;   Example: $FFEA (TIC3/24X crank) → JMP $AAC5 (actual ISR handler)
...
;   - Demonstrates the crank period injection technique
...
PERIOD_24X_RAM EQU $194C       ; crank period storage (VERIFIED: 1W @ 0x101E1) ; Verified: CRANK_PERIOD_24X (5 refs bank 2 both. TIC3 ISR variable) [Enhanced-fix]
...
;
            ...
            ORG $C468          ; Free space VERIFIED: 15,192 bytes (was $18156 WRONG!) ; FIXED: $14468 is a FILE OFFSET, not CPU addr. CPU=$C468 bank 2 (file 0x14468) [Enhanced-fix]

;==============================================================================
; IGNITION CUT MAIN HANDLER
;==============================================================================
; This routine is called INSTEAD of the stock "STD     $194C" instruction at
...
;
; 2. Extract binary from S19 file and inject into stock binary at 0x0C500
;
; 3. Modify hook point at 0x101E1:
;    Original: FD 01 7B  (STD $017B — dwell intermediate)
;    Modified: BD C5 00  (JSR $C500 — jump to patch in engine bank free space)
;
...
; The hook at 0x101E1 intercepts the crank period write to inject fake values
;
;------------------------------------------------------------------------------
...
;   $00E2 = TIC3 (24X Crank) interrupt handler ← YOUR INJECTION POINT
...
; $017B = dwell intermediate storage (NOT crank period!)
;
...
; HOOK POINT PATCH:
;   File offset: 0x101E1
;   Original: FD 01 7B = STD     $194C (store D to 3X period)
